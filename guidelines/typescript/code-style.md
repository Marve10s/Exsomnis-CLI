...
- Application code imports through the `@/` alias, which `tsconfig.json` maps to `apps/exsomnis/src`, so `import { ThreadId } from '@/domain/ids.ts'` works from any depth. Relative imports are not used in `apps/exsomnis`. Bun and `bun build --compile` read the alias from `tsconfig.json`.
- Imports carry the `.ts` extension. Node's ESM loader, which oxlint uses to load the lint plugins, requires it, and `allowImportingTsExtensions` is on.
